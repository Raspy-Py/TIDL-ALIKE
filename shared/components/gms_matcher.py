...
class GmsMatcher:
    ...
    def compute_matches(self, img1, img2):
        ...
        # print(f"[keypoints] type: {type(self.keypoints_image1[0])}, len: {len(self.keypoints_image1)}")
        # print(f"[descriptors] type: {type(descriptors_image1)}, shape: {descriptors_image1.shape}")
    ...
    def match_gms(self, prev_ktps, prev_desc, curr_ktps, curr_desc, img_size):
        # self.keypoints_image1, descriptors_image1 = self.descriptor.detectAndCompute(img1, np.array([]))
        # self.keypoints_image2, descriptors_image2 = self.descriptor.detectAndCompute(img2, np.array([]))
        # print(f"[keypoints] type: {type(self.keypoints_image1[0])}, len: {len(self.keypoints_image1)}")
        # print(f"[descriptors] type: {type(descriptors_image1)}, shape: {descriptors_image1.shape}")
        ...
        self.keypoints_image1 = prev_ktps
        self.keypoints_image2 = curr_ktps

        if self.gms_matches:
            self.empty_matches()

        all_matches = self.matcher.match(prev_desc, curr_desc)
        self.normalize_points(prev_ktps, img_size, self.normalized_points1)
        self.normalize_points(curr_ktps, img_size, self.normalized_points2)
        self.matches_number = len(all_matches)
        self.convert_matches(all_matches, self.matches)
        self.initialize_neighbours(self.grid_neighbor_left, self.grid_size_left)
