...
def resize_and_pad(image, model_w, model_h):
    ...
    padded = np.pad(resized, ((0, model_h - new_h), (0, model_w - new_w), (0, 0)))

    return np.expand_dims(np.transpose(padded, (2, 0, 1)), axis=0)

def unpad_output(descriptors_map, scores_map, original_size):
    o_h, o_w = original_size
    scale = min(model_shape[0] / o_h, model_shape[1] / o_w)
    new_h, new_w = int(o_h * scale), int(o_w * scale)

    return (descriptors_map[:, :, :new_h, :new_w],
            scores_map[:, :, :new_h, :new_w])
...
class ALIKEPipeline:
    def __init__(self, n_desc=200):
        ...
        # DKD
        self.sort = False
        self.dkd = DKD(radius=2, top_k=n_desc, detector="maxpool")
