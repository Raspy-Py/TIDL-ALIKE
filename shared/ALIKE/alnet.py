...
class ALNet(nn.Module):
    ...
    def forward(self, image):
        ...
        # replaced pool4 by 2 successive pool2
        x3 = self.pool2(x2)
        x3 = self.pool2(x3)
        ...
        x3_up = self.upsample4(self.upsample2(x3))  # B x dim//4 x H x W
        ...
        # Split version
        descriptor_map, scores_map = torch.split(x, x.size(1) - 1, dim=1)
        #scores_map = torch.sigmoid(scores_map.squeeze(0))
        scores_map = torch.sigmoid(scores_map)

        #return scores_map, descriptor_map.squeeze(0)
