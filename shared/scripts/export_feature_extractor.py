...
if __name__ == "__main__":
    ...
    feature_extractor = FeatureExtractor(**CONFIGS[args.config], device=DEVICE, manual_norm=True)

    print(f"Exporting model to ONNX.")
    model_file = args.file + ".onnx"
    temp_name = os.path.join(EXPORT_FOLDER, model_file if args.cut == "" else "temp_" + model_file)
    torch.onnx.export(
        feature_extractor, image, temp_name,
        input_names=[args.input], 
        output_names=['descriptor_map', 'scores_map'],
