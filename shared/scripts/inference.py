...
class FeatureExtractor(object):
    def __init__(self, model_path: str, artifacts_path: str = None):
        ...
        if self.artifacts_folder is not None:
...
from components.utils import save_tensor_to_file

if __name__ == "__main__":
    feature_extrator = FeatureExtractor("./assets/models/small_3_sq.onnx")
    
    image = cv2.imread("assets/images/input_image.png")
    image = cv2.resize(image, (256, 192), interpolation=cv2.INTER_LINEAR)
    image = np.pad(image, ((32, 32), (0, 0), (0, 0)), mode='constant', constant_values=0)
    image = preprocess_image(image)

    descriptor_map, scores_map = feature_extrator.run(image)

    save_tensor_to_file(descriptor_map, "./assets/images/python_descriptor_map.tensor")
    save_tensor_to_file(scores_map, "./assets/images/python_scores_map.tensor")
